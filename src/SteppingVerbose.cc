...
void SteppingVerbose::TrackingStarted() {  
  ...
  if (verboseLevel > 0) {
    ...
    G4cout << std::setw( 5) << fTrack->GetCurrentStepNumber() << " "
	   ...
	   << std::setw(10) << fTrack->GetVolume()->GetName() << "   initStep" << G4endl;        
  }
...
void SteppingVerbose::StepInfo() {  
  ...
  if (verboseLevel >= 1) {
    ...
    G4cout << "   " << std::setw(10) << procName << G4endl;
	...
	G4cout << "\n    :----- List of secondaries ----------------" << G4endl;
        ...
        for (size_t lp1=(*fSecondary).size()-tN2ndariesTot; lp1<(*fSecondary).size(); lp1++) {
          ...
          G4cout << "   "        << std::setw(13) << (*fSecondary)[lp1]->GetDefinition()->GetParticleName() << ":"
		 << "  energy =" << std::setw( 6) << G4BestUnit((*fSecondary)[lp1]->GetKineticEnergy(), "Energy") 
		 << "  time ="   << std::setw( 6) << G4BestUnit((*fSecondary)[lp1]->GetGlobalTime(),    "Time") << G4endl;
        }              
        G4cout << "    :------------------------------------------\n" << G4endl;
      }
    }    
